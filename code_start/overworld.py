...
class Overworld:
    ...
    def create_path_sprites(self):
        ...
        for path_id, data in self.paths.items():
            ...
            for index, points in enumerate(path):
                if index < len(path) - 1 :
                    ...
                    path_dir = (end - start) / TILE_SIZE    # give next tile Ok, Himanshu Sonkar
                    start_tile = vector(int(start[0] / TILE_SIZE), int(start[1]/ TILE_SIZE))  # start tile
        ...
        for key, path in path_tiles.items():
            for index, tile in enumerate(path):  # Always check start tile , middle tile and end tile
    ...
    def input(self):
        ...
        if self.current_node and not self.icon.path :  # if player is in a spot and not moving , then only check for keys
    ...
    def move(self, direction):
        path_key = int(self.current_node.paths[direction][0])  # since path return a string
        path_reverse = True if self.current_node.paths[direction][-1] == 'r' else False
        # print(path_key)
        paths = self.paths[path_key]['pos'][:] if not path_reverse else self.paths[path_key]['pos'][::-1] #reverse a list in python
        #print('path')
        self.icon.start_move(paths)
