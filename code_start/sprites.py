...
class Sprite(pygame.sprite.Sprite):
	def __init__(self,pos,surf = pygame.Surface((TILE_SIZE,TILE_SIZE)), groups=None, z= Z_LAYERS['main']):  # capturing via a parameter from level
		super().__init__(groups)
		self.image= surf #then assign to image
...
class AnimatedSprite(Sprite):
	def __init__(self, pos, frames, groups, z=Z_LAYERS['main'], animation_speed = ANIMATION_SPEED):
		self.frames, self.frame_index = frames, 0              #self.frame -> list of surfaces
...
class MovingSprite(AnimatedSprite):
	def __init__(self, frames, groups, start_pos, end_pos, move_dir, speed, flip = False):
		...
		# self.image.fill('white')
		# in level.py we can create the moving sprite
		if move_dir=='x':
	...
	# logic function for sprites to move
	def update(self, dt):
		self.old_rect = self.rect.copy()
...
#to create player for overworld
