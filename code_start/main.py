...
# to run basic game logic
class Game:
	def __init__(self):
		#initialize attributes
		pygame.init()
		self.display_surface = pygame.display.set_mode((WINDOW_WIDTH,WINDOW_HEIGHT))
		pygame.display.set_caption("Jolly Roger's Isle")
		...
		self.bg_music.play(-1)   # plays the bg music for entire game , -1 loops the same file again and again
	...
	# to check if player runs out of health
	def check_game_over(self): # can be modified to much more elegant way
	...
	def run(self):
		while True:
			# concept of delta time for frame rate adjustment
			dt = self.clock.tick()/1000
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					pygame.quit()
					sys.exit()
			# below is run method of level
