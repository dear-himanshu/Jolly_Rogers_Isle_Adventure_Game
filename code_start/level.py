...
class Level:
	def __init__(self, tmx_map, level_frames, audio_files ,data, switch_stage):
		...
		tmx_level_properties = tmx_map.get_layer_by_name('Data')[0].properties  # since it contains a single object
	...
	def setup(self, tmx_map, level_frames, audio_files):
		# here we are importing terrain ,and all other required graphics and then we pass these to respective modules
		#imported tiles

		for layer in ['BG' , 'Terrain' , 'FG' , 'Platforms']:
			for x,y, surf in tmx_map.get_layer_by_name(layer).tiles(): #creates all sprits , we are getting surface passing into sprite
		...
		# importing bg objects
		for obj in tmx_map.get_layer_by_name('BG details'):
			# if obj has name --> its is animated else if static that means its not animated
			if obj.name == 'static':
				#Sprite() function creates all the sprites
		...
		# here we are importing object
		#objects
		for obj in tmx_map.get_layer_by_name('Objects'):
			# print(obj)
			if obj.name == 'player':
				#important point that player is not the part of collision_sprites
				self.player = Player(
					pos=(obj.x,obj.y),
					groups=self.all_sprites,
				...
				if obj.name in ('barrel', 'crate'):
					...
					# animation speed to have random animation for all palm trees
		...
		for obj in tmx_map.get_layer_by_name('Moving Objects'):
			# print(obj)
			#here we care about helicopter# import all the images inside folder and places inside a list
			if obj.name == 'spike':
				# need trignometry here, important
				Spike(
				...
				speed = obj.properties['speed']
				# here we have all the properties build , now declare class to actually
				# show them in the game , and we make class in sprites.py
	...
	def pearl_collision(self): #to kill a pearl sprite if it collides with a wall etc or say collision sprite
	...
	def item_collision(self):
		if self.item_sprites:
			item_sprites = pygame.sprite.spritecollide(self.player, self.item_sprites, True)  # function to kill sprite and check overlap
			if item_sprites:
	...
	def attack_collision(self):  # When you attack pearl or tooth , it will reverse there movement
	...
	def check_constraint(self):
		...
		#Player reaching the flag
		if self.player.hitbox_rect.colliderect(self.level_finish_rect):
			# print('success')
			self.switch_stage('overworld', self.level_unlock)

	def run(self,dt):
		# this update will also call update function in player.py and all other groups
