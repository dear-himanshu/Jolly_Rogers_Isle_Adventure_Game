...
class Player(pygame.sprite.Sprite):
	# here we are creating player properties and further in level class we export it there
	...
	def input(self):
		...
		if not self.timers['wall jump'].active:
			...
			#length of the vector always going to be 1
			self.direction.x = input_vector.normalize().x if input_vector else input_vector.x

		#checking for spacae
		if keys[pygame.K_SPACE]:
			self.jump = True  # we can use this  in move method
	...
	def move(self,dt):
		# taking current position of the player and increasing its speed in certain
		# direction
		#horizontal logic
	...
	def check_contact(self):
		floor_rect = pygame.Rect(self.hitbox_rect.bottomleft,(self.hitbox_rect.width,2)) #assigning it to player's bottom
